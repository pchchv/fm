...
func runSearch(t *testing.T, ignorecase, smartcase, ignorediacritics, smartdiacritics bool, base, pattern string, expected bool) {
	genOpts.ignorecase = ignorecase
	genOpts.smartcase = smartcase
	genOpts.ignoredia = ignorediacritics
	genOpts.smartdia = smartdiacritics
	matched, _ := searchMatch(base, pattern)
	if matched != expected {
		t.Errorf("False search for" +
			" ignorecase = " + strconv.FormatBool(genOpts.ignorecase) + ", " +
			" smartcase = " + strconv.FormatBool(genOpts.smartcase) + ", " +
			" ignoredia = " + strconv.FormatBool(genOpts.ignoredia) + ", " +
			" smartdia = " + strconv.FormatBool(genOpts.smartdia) + ", ")
	}
}
