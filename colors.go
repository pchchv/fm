...
func parseStyles() styleMap {
	...
	// ex   EXEC                   01;32
	//
	// (Entries marked with * are not implemented in fm)
	...
	if env := os.Getenv("FM_COLORS"); env != "" {
		sm.parseGNU(env)
	}

	for _, path := range genColorsPaths {
...
func applyAnsiCodes(s string, st tcell.Style) tcell.Style {
	...
	for _, tok := range toks {
		...
		if err != nil {
			golog.Info("converting escape code: %s", err)
			continue
		}
		nums = append(nums, n)
	}
	...
	for i := 0; i < len(nums); i++ {
		...
		case n == 38:
			if i+3 <= len(nums) && nums[i+1] == 5 {
				...
				golog.Info("unknown ansi code or incorrect form: %d", n)
			}
		case n >= 40 && n <= 47:
		...
		default:
			golog.Info("unknown ansi code: %d", n)
		}
	}
...
func (sm styleMap) parseFile(path string) {
	...
	f, err := os.Open(path)
	if err != nil {
		golog.Info("opening colors file: %s", err)
		return
	}
	defer f.Close()
	...
	if err != nil {
		golog.Info("reading colors file: %s", err)
		return
	}
...
// parseGNU parses $LS_COLORS environment variable.
func (sm styleMap) parseGNU(env string) {
	for _, entry := range strings.Split(env, ":") {
		...
		if len(pair) != 2 {
			golog.Info("invalid $LS_COLORS entry: %s", entry)
			return
		}
...
// parseBSD parses $LSCOLORS environment variable.
func (sm styleMap) parseBSD(env string) {
	if len(env) != 22 {
		golog.Info("invalid $LSCOLORS variable: %s", env)
		return
	}
	...
	getStyle := func(r1, r2 byte) tcell.Style {
		...
		default:
			golog.Info("invalid $LSCOLORS entry: %c", r1)
		...
		default:
			golog.Info("invalid $LSCOLORS entry: %c", r2)
			return tcell.StyleDefault
		}
