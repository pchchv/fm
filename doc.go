...
The documentation can be read from the terminal with the command 'fm -doc' or
online at https://pkg.go.dev/github.com/pchchv/fm

You can also use the command 'doc' (default '<f-1>') command inside fm to view the documentation in the pager.
...
is provided by fm. This command is used to simulate key pushes given as its
...
# Remote Commands
One of the more advanced features in fm is remote commands. All clients connect
...
# File Operations
fm uses its own builtin copy and move operations by default. These are
...
fm falls back to copying and then deletes the original files if there are no
...
on multiple files by selecting them first. When you 'copy' a file, fm doesn't
...
By default, fm does not assign 'delete' command to a key to protect new users.
...
# Searching Files
There are two mechanisms implemented in fm to search a file in the current
...
For coloring fm recognizes ansi escape codes.
...
for big files. fm automatically closes the previewer script output pipe with a
...
information.
# Changing Directory
fm changes the working directory of the process to the current directory so
...
# Colors
fm tries to automatically adapt its colors to the environment. It starts with
...
This can be useful since there are some differences between ls and fm, though
...
of line.
You can configure fm colors in two different ways. First, you can only configure
8 basic colors used by your terminal and fm should pick up those colors
...
above for fine grained customization. Note that 'LS_COLORS/FM_COLORS' are more
...
It is worth noting that fm uses as many colors advertised by your terminal's
...
Default fm colors are mostly taken from GNU dircolors defaults. These defaults
use 8 basic colors and bold attribute. Default dircolors entries with background
colors are simplified to avoid confusion with current file selection in fm.
...
# Icons
Icons are configured using 'FM_ICONS' environment variable or an icons file.
The variable uses the same syntax as 'LS_COLORS/FM_COLORS'. Instead of colors,
