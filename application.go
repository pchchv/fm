...
func (app *app) quit() {
	if genOpts.history {
		if err := app.writeHistory(); err != nil {
			golog.Info("writing history file: %s", err)
		}
	}
	if !genSingleMode {
		if err := remote(fmt.Sprintf("drop %d", genClientID)); err != nil {
			golog.Info("dropping connection: %s", err)
		}
		if genOpts.autoquit {
			if err := remote("quit"); err != nil {
				golog.Info("auto quitting server: %s", err)
			}
		}
	}
}
...
func (app *app) readFile(path string) {
	golog.Info("reading file: %s", path)
...
// Expressions are read from the client and server in
// separate goroutines and sent here over channels for evaluation.
// Similarly, directories and regular files are also read in
// separate goroutines and sent here to be updated.
func (app *app) loop() {
	...
	if genConfigPath != "" {
		if _, err := os.Stat(genConfigPath); !os.IsNotExist(err) {
			app.readFile(genConfigPath)
		} else {
			golog.Info("config file does not exist: %s", err)
		}
	} else {
		for _, path := range genConfigPaths {
	...
	if genSelect != "" {
		go func() {
			lstat, err := os.Lstat(genSelect)
			if err != nil {
				app.ui.exprChan <- &callExpr{"echoerr", []string{err.Error()}, 1}
			} else if lstat.IsDir() {
				app.ui.exprChan <- &callExpr{"cd", []string{genSelect}, 1}
			} else {
				app.ui.exprChan <- &callExpr{"select", []string{genSelect}, 1}
			}
		}()
	}
	...
	for {
		...
		case <-app.quitChan:
			...
			if cmd, ok := genOpts.cmds["on-quit"]; ok {
				cmd.eval(app, nil)
			}
			...
			if genLastDirPath != "" {
				f, err := os.Create(genLastDirPath)
				if err != nil {
					golog.Info("opening last dir file: %s", err)
				}
				defer f.Close()

				_, err = f.WriteString(app.nav.currDir().path)
				if err != nil {
					golog.Info("writing last dir file: %s", err)
				}
			}
...
// runShell is used to run a shell command.
...
func (app *app) runShell(s string, args []string, prefix string) {
	...
	case "%":
		...
		if err != nil {
			golog.Info("writing stdin: %s", err)
		}
		app.cmdIn = stdin
		stdout, err := cmd.StdoutPipe()
		if err != nil {
			golog.Info("reading stdout: %s", err)
		}
		out = stdout
	...
	case "%":
		...
		go func() {
			...
			if err := cmd.Wait(); err != nil {
				golog.Info("running shell: %s", err)
			}
			app.cmd = nil
			app.ui.cmdPrefix = ""
...
func loadFiles() (list []string, cp bool, err error) {
	...
	golog.Info("loading files: %v", list)

	return
}

func saveFiles(list []string, cp bool) error {
	if err := os.MkdirAll(filepath.Dir(genFilesPath), os.ModePerm); err != nil {
		return fmt.Errorf("creating data directory: %s", err)
	}

	files, err := os.Create(genFilesPath)
	if err != nil {
		return fmt.Errorf("opening file selections file: %s", err)
	}
	defer files.Close()

	golog.Info("saving files: %v", list)
