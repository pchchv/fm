...
func (e *callExpr) eval(app *app, args []string) {
	...
	case "open":
		...
		if genSelectionPath != "" {
			out, err := os.Create(genSelectionPath)
			if err != nil {
				golog.Info("opening selection file: %s", err)
				return
			}
			defer out.Close()
			...
			if err != nil {
				golog.Info("writing selection file: %s", err)
			}
		...
		if cmd, ok := genOpts.cmds["open"]; ok {
			cmd.eval(app, e.args)
		}
	case "jump-prev":
		resetIncCmd(app)
	...
	case "paste":
		...
		if cmd, ok := genOpts.cmds["paste"]; ok {
			cmd.eval(app, e.args)
	...
	case "delete":
		...
		if cmd, ok := genOpts.cmds["delete"]; ok {
			cmd.eval(app, e.args)
			app.nav.unselect()
			if genSingleMode {
				app.nav.renew()
	...
	case "rename":
		...
		if cmd, ok := genOpts.cmds["rename"]; ok {
			cmd.eval(app, e.args)
			if genSingleMode {
				app.nav.renew()
				app.ui.loadFile(app, true)
	...
	case "cmd-enter":
		...
		case "$":
			golog.Info("shell: %s", s)
			app.ui.cmdPrefix = ""
			app.cmdHistory = append(app.cmdHistory, cmdItem{"$", s})
			app.runShell(s, nil, "$")
		case "%":
			golog.Info("shell-pipe: %s", s)
			app.cmdHistory = append(app.cmdHistory, cmdItem{"%", s})
			app.runShell(s, nil, "%")
		case ">":
			...
			app.cmdOutBuf = nil
		case "!":
			golog.Info("shell-wait: %s", s)
			app.ui.cmdPrefix = ""
			app.cmdHistory = append(app.cmdHistory, cmdItem{"!", s})
			app.runShell(s, nil, "!")
		case "&":
			golog.Info("shell-async: %s", s)
			app.ui.cmdPrefix = ""
			app.cmdHistory = append(app.cmdHistory, cmdItem{"&", s})
			app.runShell(s, nil, "&")
		case "/":
		...
		default:
			golog.Info("entering unknown execution prefix: %q", app.ui.cmdPrefix)
		}
...
func (e *execExpr) eval(app *app, args []string) {
	switch e.prefix {
	case "$":
		golog.Info("shell: %s -- %s", e, args)
		app.runShell(e.value, args, e.prefix)
	case "%":
		golog.Info("shell-pipe: %s -- %s", e, args)
		app.runShell(e.value, args, e.prefix)
	case "!":
		golog.Info("shell-wait: %s -- %s", e, args)
		app.runShell(e.value, args, e.prefix)
	case "&":
		golog.Info("shell-async: %s -- %s", e, args)
		app.runShell(e.value, args, e.prefix)
	default:
		golog.Info("evaluating unknown execution prefix: %q", e.prefix)
	}
}
...
func update(app *app) {
	...
	switch {
	case genOpts.incsearch && app.ui.cmdPrefix == "/":
	...
	case genOpts.incfilter && app.ui.cmdPrefix == "filter: ":
...
func restartIncCmd(app *app) {
	if genOpts.incsearch && (app.ui.cmdPrefix == "/" || app.ui.cmdPrefix == "?") {
		dir := app.nav.currDir()
		app.nav.searchInd = dir.ind
		app.nav.searchPos = dir.pos
		update(app)
	} else if genOpts.incfilter && app.ui.cmdPrefix == "filter: " {
		dir := app.nav.currDir()
		app.nav.prevFilter = dir.filter
		update(app)
	}
...
func insert(app *app, arg string) {
	switch {
	case genOpts.incsearch && (app.ui.cmdPrefix == "/" || app.ui.cmdPrefix == "?"):
	...
	case app.ui.cmdPrefix == "find: ":
		...
		if genOpts.findlen == 0 {
			...
			if len(app.nav.find) < genOpts.findlen {
				app.ui.cmdAccLeft = append(app.ui.cmdAccLeft, []rune(arg)...)
				return
			}
