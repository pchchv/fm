...
var (
	genThisYear = time.Now().Year()
	gValKey     map[string]tcell.Key
	genKeyVal   = map[tcell.Key]string{
...
func (ui *ui) echoerr(msg string) {
	ui.msg = fmt.Sprintf(genOpts.errorfmt, msg)
	golog.Info("error: %s", msg)
}
...
func (ui *ui) loadFileInfo(nav *nav) {
	...
	ui.echof("%v %v%v%v%4s %v%s",
		...
		humanize(curr.Size()),
		curr.ModTime().Format(genOpts.timefmt),
		linkTarget)
}
...
func (ui *ui) drawPromptLine(nav *nav) {
	...
	if strings.HasPrefix(pwd, genUser.HomeDir) {
		pwd = filepath.Join("~", strings.TrimPrefix(pwd, genUser.HomeDir))
	}
	...
	prompt = strings.Replace(genOpts.promptfmt, "%u", genUser.Username, -1)
	prompt = strings.Replace(prompt, "%h", genHostname, -1)
...
func (ui *ui) readNormalEvent(ev tcell.Event, nav *nav) expr {
	...
	case *tcell.EventKey:
		...
		binds, ok := findBinds(genOpts.keys, string(ui.keyAcc))
		...
		default:
			if ok {
				if len(ui.keyCount) > 0 {
					...
					if err != nil {
						golog.Info("converting command count: %s", err)
					}
					count = c
				}
				expr := genOpts.keys[string(ui.keyAcc)]
	...
	case *tcell.EventMouse:
		...
		if expr, ok := genOpts.keys[button]; ok {
			return expr
		}
		...
		var dir *dir
		if genOpts.preview && wind == len(ui.wins)-1 {
			curr, err := nav.currFile()
			if err != nil {
				return nil
			} else if !curr.IsDir() || genOpts.dirpreviews {
	...
	case *tcell.EventError:
		golog.Info("Got EventError: '%s' at %s", tev.Error(), tev.When())
	case *tcell.EventInterrupt:
		golog.Info("Got EventInterrupt: at %s", tev.When())
	}
	return nil
}
...
func getWidths(wtot int) []int {
	rsum := 0
	for _, r := range genOpts.ratios {
		rsum += r
	}

	wlen := len(genOpts.ratios)
	widths := make([]int, wlen)

	wsum := 0
	for i := 0; i < wlen-1; i++ {
		widths[i] = genOpts.ratios[i] * wtot / rsum
		wsum += widths[i]
	}
	...
	case *tcell.EventKey:
		if tev.Key() == tcell.KeyRune {
			if tev.Modifiers() == tcell.ModMask(tcell.ModAlt) {
				...
				if expr, ok := genOpts.cmdkeys[val]; ok {
					return expr
				}
			} else {
...
func listBinds(binds map[string]expr) *bytes.Buffer {
	...
	t.Init(b, 0, genOpts.tabstop, 2, '\t', 0)
